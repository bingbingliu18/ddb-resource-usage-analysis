...
LOG_DIR = "logs"
JSON_LOG_FILE = os.path.join(LOG_DIR, "ddb_resource_logs.json")
...
def write_logs_to_file(logs):
    """Write logs to local files in both JSON and NDJSON formats"""
    if not logs:
        return True
    ...
    try:
        ...
        logging.info(f"Successfully wrote {len(logs)} logs to local files")
...
def generate_and_write_logs(module_type):
    """Generate logs for a specific module and write them to local files"""
    logs_buffer = []
...
def print_statistics():
    ...
    while True:
        ...
        if os.path.exists(JSON_LOG_FILE):
            json_size = os.path.getsize(JSON_LOG_FILE) / (1024 * 1024)  # Size in MB
            logging.info(f"JSON log file size: {json_size:.2f} MB")
...
def main():
    ...
    logging.info("Starting DynamoDB resource usage log generator (Local File Version)")
    ...
    logging.info(f"Logs will be written to {JSON_LOG_FILE} and {NDJSON_LOG_FILE}")
    ...
    try:
        ...
        if os.path.exists(JSON_LOG_FILE):
            json_size = os.path.getsize(JSON_LOG_FILE) / (1024 * 1024)  # Size in MB
            logging.info(f"Final JSON log file size: {json_size:.2f} MB")
