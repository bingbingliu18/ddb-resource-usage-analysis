...
def get_open_games_by_map(map_name):
    """
    Get open games by map name using the MapPKIndex GSI.
    ...
    """
    try:
        # Use query with MapPKIndex GSI to find open games for the specified map
        logger.info(f"Querying for open games on map '{map_name}' using MapPKIndex GSI...")
...
def main():
    """
    Main function: Randomly select a map, query for open games on that map using MapPKIndex GSI,
    ...
    try:
        ...
        # Get open games on the selected map using MapPKIndex GSI
